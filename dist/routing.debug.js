  ...
  handleHashChange = function(e) {
    ...
    for (k = 0, len2 = matchingRoutes.length; k < len2; k++) {
      ...
      if (route.router.current.path !== path) {
        route.router._go(route, path, true, 'hashchange');
      }
    }
  };
...
module.exports = Route = (function() {
  ...
  Route.prototype._invokeAction = function(action, relatedPath, relatedRoute, navDirection) {
    var result;
    if (action === this._enterAction) {
      debug("entering '" + this.path.original + "' from '" + relatedPath + "'");
    }
    result = action.call(this._context, relatedPath, relatedRoute, navDirection);
...
helpers.parseQuery = function(path, parser) {
  var j, key, len, pair, pairs, parsed, query, split, value;
...
helpers.serializeQuery = function(query, serializer) {
  var j, key, keys, len, output, value;
  output = '';
