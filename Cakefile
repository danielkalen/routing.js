...
task 'build:js', (options)->
	...
	Promise.resolve()
		.then ()-> {src:"src/index.coffee", dest:"build/routing#{debug}.js"}
		.tap ()-> console.log 'compiling js' unless global.silent
		.then (file)-> compileJS(file, debug:options.debug, umd:'Routing', target:'browser')
...
task 'measure', (options)->
	Promise.resolve()
		...
		.then ()->
			...
			measure {debug:"./#{DIR}/routing.debug.js", release:"./#{DIR}/routing.js"}
